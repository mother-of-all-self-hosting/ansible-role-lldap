...
#
# Tags look like `v<LLDAP version>-<release>`, matching what this repository
# has always published (`v0.6.2-7`, `v0.6.3-0`, ...):
#
# - if defaults/main.yml points at an LLDAP version that has never been
#   released, the release counter restarts at 0 (`v0.6.4-0`)
# - otherwise the counter is incremented (`v0.6.3-3`), but only if something
...
# Anchored on `lldap_version:` so that neither the derived
# `lldap_container_image_self_build_repo_version:` nor a commented-out line
# can be mistaken for it.
version="$(sed -nE 's|^lldap_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the LLDAP version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v0.6.3`),
